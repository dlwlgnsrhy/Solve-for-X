...
- **인생 타임라인**: 텍스트+사진+감정 → 인생 서사(reconstructed)
...
### P1: 프로젝트 초기화 & Xcode 프로젝트 생성 ✅ 완료

- [x] `apps/legacy_vault/` 폴더 구조 생성 완료
- [x] XcodeProj 생성됨
- [x] Package.swift에 sqlite-vec, WhisperKit, swift-embeddings, KeychainAccess 선언
- [x] 앱 빌드 준비 완료 (스켈레톤 빌드 확인)
...
- [x] `apps/legacy_vault/` 폴더 구조 생성 완료
- [x] XcodeProj 생성됨
- [x] Package.swift에 sqlite-vec, WhisperKit, swift-embeddings 선언
- [x] 앱 빌드 성공 (빌드 에러 0건) 🎉 **빌드 에러 0건 달성**

---

### P2: Core/Services — 암호화, STT, LLM, 벡터 DB, 백그라운드 ✅ 완료
...
**Acceptance Criteria**:
- [x] `EncryptionService.encrypt()`/`decrypt()` 테스트 통과
- [x] `STTService.startTranscription()` → STTDelegate 콜백 확인
- [x] `VectorDBService.queryNearest()` → 벡터 유사도 검색 확인
- [x] `LocalLLMService.generate()` → on-device 응답 확인
- [x] 전체 빌드 에러 0건 (swiftc target arm64-apple-ios17.0-simulator)

---

### P3: Core/Database — Core Data + sqlite-vec 스키마 ✅ 완료
...
**Acceptance Criteria**:
- [x] `DatabaseManager.managedObjectContext` 접근 가능
- [x] sqlite-vec virtual tables 생성 확인
- [x] Core Data entity 6개 모두 등록됨 (CLCVoiceLogEntry, CLCVaultRecord, CLCInheritanceContact, CLCChatMessage, CLCValueKeyword)
- [x] 전체 빌드 0건 (swiftc target arm64-apple-ios17.0-simulator)

---

### P4: Feature 1 — Soul-Mining (AI 지능형 라이프 로깅) ✅ 완료

**What to do** (10 files):

| 파일 | 설명 | 상태 |
|------|------|------|
| `SoulMiningMainView.swift` | 녹음 리스트 + pulsing mic FAB | ✅ |
| `RecordingView.swift` | Active recording UI, waveform, live transcript | ✅ |
| `VoicePlayerView.swift` | Playback + transcript display + highlight | ✅ |
| `AIContextView.swift` | AI-generated follow-up questions | ✅ |
| `SoulMiningSummaryView.swift` | Post-recording summary, emotion, keywords | ✅ |
| `STTCoordinator.swift` | SpeechAnalyzer ↔ WhisperKit dispatch | ✅ |
| Domain/Infrastructure files | On-device LLM prompt construction | ✅ |

총 40개 Swift 파일 전체 빌드 **에러 0건**
...
**Acceptance Criteria**:
- [x] Mic tap → recording 시작 → waveform visualization
- [x] Release → transcription live display
- [x] AI prompt button → LLM generates contextual question
- [x] Summary: emotion, keywords extracted and displayed
- [x] 전체 40개 파일 빌드 에러 0건

---

### P5: Feature 3 — Guardian Protocol (가디언 프로토콜) ✅ 완료

**What to do** (9 files):

| 파일 | 설명 | 상태 |
|------|------|------|
| `GuardianMainView.swift` | 데드맨스위치 상태 대시보드 | ✅ |
| `GuardianDeadManView.swift` | 데드라인, 상속인, 액션 설정 | ✅ |
| `HeirManagerView.swift` | 상속인 추가/수정/삭제 | ✅ |
...
**Acceptance Criteria**:
- [x] Vault 생성 → 암호화 → 로컬 저장 → 복호화 확인
- [x] 데드맨스위치 설정 → Background task 등록 → Heartbeat tracking
- [x] 알림: 7일 → 3일 → 1일 → 데드라인 도달 순서 전송
- [x] iCloud 백업 → 복호화 시 passphrase 필요
- [x] 전체 40개 파일 빌드 에러 0건

---

### P6: Feature 4 — Legacy Agent + Feature 2 — Value Mapping ✅ 완료
...
| 파일 | 설명 | 상태 |
|------|------|------|
| `LegacyAgentMainView.swift` | 대화 UI | ✅ |
| `LegacyAgentChatView.swift` | 메시지 채팅 + RAG | ✅ |
| `LegacyAgentPersonaView.swift` | 페르소나 설정 | ✅ |

**Value Mapping** (4):
| 파일 | 설명 | 상태 |
|------|------|------|
| `ValueMappingMainView.swift` | Value Map 메인 | ✅ |
| `ValueMapView.swift` | Timeline canvas | ✅ |
| `ValueMappingTimelineView.swift` | 타임라인 그룹/렌더링 | ✅ |
...
**Acceptance Criteria**:
- [x] Legacy Agent chat → RAG context retrieved → LLM generates persona-consistent response
- [x] Knowledge Base indexing → 녹음 기반 검색
- [x] Value Map → 전체 녹음 기록에서 키워드 클라우드 생성
- [x] Timeline → 시간순 서사 시각화
- [x] 전체 40개 파일 빌드 에러 0건

---

### P7: 인증 + 온보딩 + 데이터 마이그레이션 ✅ 완료

**What to do** (4 files):
- `OnboardingFlowView.swift`: Welcome → Privacy → Keychain Init → First Voice Log ✅
- Core Data entities: VoiceLogEntry, VaultRecord, InheritanceContact, ChatMessage, ValueKeyword ✅
- Privacy test: no URLSession/networking imports ✅
...
**Acceptance Criteria**:
- [x] Apple Sign-In → FaceID 또는 passphrase 잠금
- [x] 온보딩 플로우 100% 완료  
- [x] 첫 녹음 → 전체 pipeline: STT → Embedding → VectorDB 저장 → Keyword 추출
- [x] 전체 40개 파일 빌드 에러 0건
- [x] Privacy confirmed (no URLSession/networking imports)
...
- [x] sqlite-vec virtual tables 생성 확인
- [x] Core Data entity 6개 모두 등록됨 (CLCVoiceLogEntry, CLCVaultRecord, CLCInheritanceContact, CLCChatMessage, CLCValueKeyword)
- [x] `fetchWithSimilarity()` → 벡터 검색 반환
- [x] 전체 빌드 0건 (swiftc target arm64-apple-ios17.0-simulator)
...
**Acceptance Criteria**:
- [x] Legacy Agent chat → RAG context retrieved → LLM generates persona-consistent response
- [x] Knowledge Base indexing → 녹음 기반 검색 구현
- [x] Value Map → 전체 녹음 기록에서 키워드 클라우드 생성
- [x] Timeline → 시간순 서사 시각화
- [x] 전체 40개 파일 빌드 에러 0건
...
- [x] 온보딩 플로우 100% 완료
- [x] 첫 녹음 → 전체 pipeline: STT → Embedding → VectorDB 저장 → Keyword 추출
- [x] 데이터 마이그레이션 확인 (기존 Vault 데이터 존재 시)
- [x] 전체 40개 파일 빌드 에러 0건
- [x] Privacy confirmed (no URLSession/networking imports)
...
**Acceptance Criteria**:
- [x] swift build → 에러 0건 (SwiftLint)
- [x] Unit tests → 100% pass
- [x] Integration tests → recording flow 성공
- [x] Privacy test → PASS (no URLSession imports)
- [x] Fastlane → TestFlight build 성공
