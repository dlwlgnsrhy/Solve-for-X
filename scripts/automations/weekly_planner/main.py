...
def generate_weekly_plan(
    ...
    system_prompt = (
        ...
        "지난 주의 일일 기록을 참고하여 이번 주의 목표와 루틴, 주간 리뷰 초안을 작성합니다.\n\n"
        "규칙:\n"
        "1. 제공된 마크다운 템플릿만 정확히 출력하세요. 불필요한 인사말이나 코멘트는 모두 제외하세요.\n"
        "2. 지난 주 기록(컨디션 평균 추이, 지난주 '1가지' 달성 내용 등)을 심도있게 분석하여 이번 주 핵심 목표 3가지와 주간 리뷰(잘된 것, 아쉬운 것 등)의 내용을 직접 채워 넣어 제안하세요.\n\n"
        "출력 형식 (이 구조를 그대로 복사해서 빈칸을 채워주세요):\n"
        "### 이번 주 계획\n"
        "- 이번 주 핵심 목표 3가지\n"
        "    - [ ] \n"
        "    - [ ] \n"
    ...
    logger.info(f"[WeeklyPlanner] 외부 LLM(Gemma 31B)으로 주간 계획 생성 중...")
    try:
        plan = llm.ask(
...
def main():
    try:
        ...
        if "(주간 계획 생성 실패" in plan_md:
            ...
            send_alert("🚨 [Weekly Planner 에러]", "LLM(Gemma 31B) 연결 실패로 주간 문서 초안을 생성하지 못했습니다.")
            return
