...
파이프라인:
  RSS 수집 → 로컬 Qwen 14B (1차 필터) → 외부 Qwen3.6 27B (요약) → Telegram
"""
...
def summarize_articles(articles: List[dict], client: LLMClient) -> List[dict]:
    """
    외부 Qwen3.6 27B로 각 기사를 3줄 한국어로 요약합니다.
    ...
    for article in articles:
        ...
        summary = client.ask(
            user_prompt=prompt,
            use_external=True,  # 외부 Qwen3.6 27B — 품질 우선
            max_tokens=150,
            temperature=0.3,
        )
...
def send_briefing(articles: List[dict], fetched_count: int, telegram: TelegramClient) -> None:
    ...
    lines.extend([
        ...
        "필터: Qwen 14B (로컬) | 요약: Qwen3.6 27B (외부)",
    ])
